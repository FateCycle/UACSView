...
namespace UACSControls
{
    public class conUnitSaddleModel
    {
        ...
        public void conInit(Panel _theBayPanel, string _theUnitNo, string _theTagServiceName, long _baySpaceX, long _baySpaceY, bool _xAxisRight, bool _yAxisDown, string _BayNo)
        {
            try
            {
        ...
        public void refreshControl()
        {
            try
            {
                //if (unitNoFlag == "CHAIN_ROW-WC")
                //{
                //    theSaddlsInfoInBay.getUnitSaddleData(true);
                //}
                //else if (unitNoFlag == "CHAIN_ROW-WR")
                //{
                ...
                //    }
                //}true
                theSaddlsInfoInBay.getUnitSaddleData();
                ...
                foreach (SaddleBase theSaddleInfo in theSaddlsInfoInBay.DicSaddles.Values.ToArray())
                {
                    ...
                    if (dicSaddleVisual.ContainsKey(theSaddleInfo.STOCK_NO))
                    {
                        theSaddleVisual = dicSaddleVisual[theSaddleInfo.STOCK_NO];
                    }
                    else
                    {
                        theSaddleVisual = new conUnitSaddle();
                        bayPanel.Invoke(new Action(() => { bayPanel.Controls.Add(theSaddleVisual);theSaddleVisual.BringToFront(); }));
                    }
                    ...
                    theSaddleVisual.Saddle_Selected += new conUnitSaddle.EventHandler_Saddle_Selected(theSaddleVisual_Saddle_Selected);
                    dicSaddleVisual[theSaddleInfo.STOCK_NO] = theSaddleVisual;
                }
            }
            catch (Exception ex)
            {
