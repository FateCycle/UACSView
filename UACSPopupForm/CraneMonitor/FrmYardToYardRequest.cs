...
namespace UACSPopupForm
{
    public partial class FrmYardToYardRequest : Form
    {
        ...
        private void BtnClose_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string sql = @"update UACS_CRANE_MANU_ORDER set COIL_NO = null,COIL_NO2 = null,FROM_STOCK_NO = null,TO_STOCK_NO = null,CMD_STATUS = 'EMPTY' ";
        ...
        private void BtnOk_Click(object sender, EventArgs e)
        {
            ...
            if (txtCoilno.Text.Trim() == "" || txtFromYard.Text.Trim() == "" || txtToYard.Text.Trim() == ""||txtCoilno2.Text.Trim ()=="" )
            {
            ...
            try
            {
                ...
                string coilNo2 = txtCoilno2.Text.Trim();
           
                string sql = @"update UACS_CRANE_MANU_ORDER  set COIL_NO = '" + coilNo + "',COIL_NO2 = '" + coilNo2+ "',FROM_STOCK_NO = '" + strFrom + "',TO_STOCK_NO = '" + strTo + "',CMD_STATUS = 'INIT' ";
                sql += " WHERE CRANE_NO = '" + CraneNo + "'";
                DBHelper.ExecuteNonQuery(sql);
            }
            catch (Exception er)
        ...
        private void btnSelect_Click(object sender, EventArgs e)
        {
            ...
            txtCoilno.Text = GetCoilNoByStockNO(strFromYard);
            txtCoilno2.Text = GetCoilNoByStockNO1(strFromYard);
        ...
        private string GetCoilNoByStockNO1(string _stockNo)
        {
            string coilNo = null;
            try
            {
        ...
        private void GetStockInfo(string _stockNo,string _coilNo)
        {
            // step 1  检查库位是否正常
                ...
                using (IDataReader rdr = DBHelper.ExecuteReader(sql))
                {
                    while (rdr.Read())
                    {
                        if (rdr["FROM_STOCK_NO"] != DBNull.Value)
	                        txtFromYard.Text = rdr["FROM_STOCK_NO"].ToString();
                        else
                            txtFromYard.Text = "";
                        ...
                        if (rdr["COIL_NO2"] != DBNull.Value)
                            txtCoilno2.Text = rdr["COIL_NO2"].ToString();
                        else
                          txtCoilno2.Text = "";

                        if (rdr["TO_STOCK_NO"] != DBNull.Value)
                        {
                            txtToYard.Text = rdr["TO_STOCK_NO"].ToString();
                        }
                        else
                            txtToYard.Text = "";
        ...
        private void FrmYardToYardRequest_Load_1(object sender, EventArgs e)
        {
