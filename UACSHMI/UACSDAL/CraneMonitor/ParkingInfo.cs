    ...
    public class ParkingInfo
    {
            ...
            try
            {
                ...
                using (IDataReader rdr = DB2Connect.DBHelper.ExecuteReader(sql))
                {
                    while (rdr.Read())
                    {
                       ...
                       // LogManager.WriteProgramLog(parkingInfo.ParkingName);
                    }
                }
            }
            catch (Exception er)
        ...
        public static void dgvStowageMessage(int theStowageId, DataGridView dgv)
        {
            ...
            try
            {
                ...
                string sql = @"SELECT  A.GROOVEID , A.MAT_NO, A.POS_ON_FRAME, A.X_CENTER, A.Y_CENTER, A.Z_CENTER, A.SLEEVE_LENGTH, A.PACKAGE_STATUS , 
                                   CASE
        ...
        public static string getStowageCarType(int theStowageId)
        {
            ...
            try
            {
                string sql = @"SELECT  CASE
                                        ...
                                        WHEN CAR_TYPE = 101 THEN '社会车辆'
                                        WHEN CAR_TYPE = 102 THEN '大头娃娃车'
                                        WHEN CAR_TYPE = 103 THEN '17m社会车辆'
                                        WHEN CAR_TYPE = 104 THEN '较低社会车辆'
                                        ELSE '其他'
