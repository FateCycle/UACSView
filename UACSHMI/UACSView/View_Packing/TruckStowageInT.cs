            ...
            try
            {
                ...
                if (cancelFlag == false)
                {
                   ...
                   // tagProvider_InStock.SetData("EV_NEW_PARKING_CARARRIVE", theOperator + "|" + date + "|" + shift + "|" + crew + "|" + parkingNO + "|" + carNO + "|" + isLoaded + "|" + headPostion + "|" + pullAbility + "|" + equno + "|" + cartype + "|" + parkingype);
                    ...
                    UACSDAL.HMILogger.WriteLog(cmd_CarArrive.Text, "车到位：" + tagCarInStosk, UACSDAL.LogLevel.Info, this.Text);
                }
            }
            catch (Exception er)
            {
            ...
            try
            {
                ...
                if (comb_ParkingNO.Text.Trim() != "请选择" || comb_ParkingNO.Text.Trim() != "")
                {
                    ...
                    if (dr == DialogResult.OK)
                    {
                        ...
                        dt_LoadMap.Clear();
                        dataGridView_LaodMap.DataSource = dt_LoadMap;
                        UACSDAL.HMILogger.WriteLog(cmd_CarLeave.Text, "车离开：" + comb_ParkingNO.Text, UACSDAL.LogLevel.Info, this.Text);
            ...
            try
            {
                ...
                if (parkNO != "请选择" || parkNO != "")
                {
                    ...
                    if (dr == DialogResult.OK)
                    {
                        ...
                        btnOperatePause.ForeColor = Color.White;
                        UACSDAL.HMILogger.WriteLog(btnOperateStrat.Text, "作业开始：" + parkNO, UACSDAL.LogLevel.Info, this.Text);
        ...
        private bool GetOperateStatus(string parkNO)
        {
            ...
            try
            {
                ...
                using (IDataReader rdr = DBHelper.ExecuteReader(SQLOder))
                {
                    ...
                    {
                        int status = 0;
                        status = UACSDAL.ManagerHelper.JudgeIntNull(rdr["PARKING_STATUS"]);
                        if (status == 170) //暂停
                        {
                            ret = true;
                            break;
