    ...
    public class UnitSaddleMethod
    {
                ...
                using (IDataReader rdr = DB2Connect.DBHelper.ExecuteReader(sql))
                {
                    while (rdr.Read())
                    {
                        ...
                        if (rdr["TAG_LOCK_REQUEST"] != System.DBNull.Value)
                        {
                            ...
                            theSaddle.TagAdd_LockRequest_FEEDBACK = theSaddle.TagAdd_LockRequest + "_FEEDBACK";
                            theSaddle.TagAdd_LockRequest_SET = theSaddle.TagAdd_LockRequest + "_SET";
                        }        
                        else
                        {
                            theSaddle.TagAdd_LockRequest = string.Empty;
                            theSaddle.TagAdd_LockRequest_FEEDBACK = string.Empty;
                            theSaddle.TagAdd_LockRequest_SET = string.Empty;
                        }
        ...
        public void getTagNameList()
        {
            try
            {
                ...
                {
                    TagNamelist.Add(theSaddle.TagAdd_LockRequest_FEEDBACK);
                    TagNamelist.Add(theSaddle.TagAdd_LockRequest_SET);
