...
{
    public partial class Form_CraneCreateOrderManager : FormBase
    {
 ...
 as Baosight.iSuperframe.Authorization.Interface.IAuthorization;
        ...
        public Form_CraneCreateOrderManager()
        {
            InitializeComponent();
        }

      
        private void Form_CraneCreateOrderManager_Load(object sender, EventArgs e)
        {
        ...
        private static void Save2ExcelSheet(DataGridView gridview, Microsoft.Office.Interop.Excel.Worksheet worksheet)
        {
            try
            {
                ...
                for (int r = 0; r < gridview.Rows.Count; r++)
                {
                    ...
                    for (int i = 0; i < gridview.ColumnCount; i++)
                    {
                        //  worksheet是从行列1开始计数的，由于第一行是行头所以为r+2,i+1
        ...
        private void butSelect_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                sqlText += "WHERE Date like '%{0}%' or TrueMan like '%{1}%' or Module like '%{2}%'";
                sqlText = string.Format(sqlText, date1, Code, TrueType);
